...
def _workspace_transition(action: Callable[..., Any]) -> Callable[..., Any]:
    @wraps(action)
    def serialized(self: ReplicationController, *args: Any, **kwargs: Any) -> Any:
        context = self._context()
        lock = getattr(context.iroh, "workspace_lock", self._workspace_lock)
        with lock:
            if self._context().closed:
                self._cancel_iroh_join()
                return done()
            if self._transition_busy:
                return done(EmitNotice(context.strings.text("network.wait_join")))
...
class ReplicationController:
    ...
    def __init__(self, ports: ReplicationPorts) -> None:
        ...
        self._joining_room_id: str | None = None

    def _context(self) -> ReplicationContext:
        return self._ports.context()

    @property
    def iroh_details(self) -> dict[str, Any]:
        if self.iroh_join_pending:
            return {**self._iroh_details, "joinPending": True}
        return self._iroh_details

    @iroh_details.setter
    def iroh_details(self, details: dict[str, Any]) -> None:
        self._iroh_details = details

    @_workspace_transition
    def restore_replication(self) -> ControllerOutcome[None]:
        context = self._context()
        if context.closed or self.iroh_join_pending:
            return done()
    ...
    def replication_mode_changed(self, selection: int | str) -> ControllerOutcome[None]:
        ...
        if not isinstance(mode, str):
            return done()
        cancelled_join = self.iroh_join_pending and mode in {"offline", "centralized"}
        if cancelled_join:
            self._cancel_iroh_join()
        elif mode == self.mode:
            return done()
    ...
    def _replication_transition_ready_value(self, mode: str) -> bool:
        ...
        if context.closed or self.iroh_join_pending:
            ...
            self._ports.apply_outcome(
                done(EmitNotice(context.strings.text("network.wait_join")))
            )
            return False
    ...
    def _saved_iroh_room_ready_value(self) -> bool:
        store = self._context().store
        connection = getattr(store, "connection", None)
        if connection is None:
            return False
        room = connection.execute(
            "SELECT room_id, conflict FROM iroh_rooms WHERE EXISTS "
        ...
        if room_id_for_secret(store.iroh_room_secret(room_id)) != room_id:
            raise ValueError("Saved Iroh room secret does not match room identity.")
        return True
    ...
    def _start_iroh_join(self, invite: Any) -> ControllerOutcome[None]:
        context = self._context()
        self.iroh_status = context.strings.text("network.status.joining_room")
        self.iroh_join_pending = True
        self._joining_room_id = invite.room_id
        self._join_generation += 1
        generation = self._join_generation
        context.iroh.join_allowed = lambda: (
            self.iroh_join_pending and generation == self._join_generation
        ...
        try:
            ...
            if self.mode == "centralized":
                context.cloud.restore()
            return self.iroh_failure(str(error))
        return done()

    def _cancel_iroh_join(self) -> None:
        if not self.iroh_join_pending:
            return
        self.iroh_join_pending = False
        self._joining_room_id = None
        self._join_generation += 1
        context = self._context()
        if context.iroh is not None:
            context.iroh.stop()
        if not context.closed:
            self.mode = context.store.replication_mode
    ...
    def leave_iroh_room(self) -> ControllerOutcome[None]:
        ...
        if answer != QMessageBox.StandardButton.Yes or self._context().closed:
            return done()
        try:
    ...
    def iroh_status_changed(self, status: str) -> ControllerOutcome[None]:
        ...
        if context.closed:
            self._cancel_iroh_join()
            return done()
        if (status == "NOT CONNECTED" and self.iroh_join_pending
                and getattr(context.iroh, "join_pending", True) is False
                and getattr(context.iroh, "join_outcome", None) != "joined"):
            self._cancel_iroh_join()
            if self.mode == "centralized":
                context.cloud.restore()
    ...
    def iroh_joined(self) -> ControllerOutcome[None]:
        ...
        if context.closed:
            self._cancel_iroh_join()
            return done()
        if not self.iroh_join_pending or getattr(context.iroh, "join_pending", False):
            return done()
        if getattr(context.iroh, "join_outcome", "joined") != "joined":
            return done()
        if (self._joining_room_id is not None
                and context.store.active_iroh_room_id != self._joining_room_id):
            return done()
        self.iroh_join_pending = False
        self._joining_room_id = None
        self.mode = "iroh"
    ...
    def iroh_failure(self, message: str) -> ControllerOutcome[None]:
        ...
        if context.closed:
            self._cancel_iroh_join()
            return done()
        if self.iroh_join_pending and getattr(context.iroh, "join_pending", False):
            return done()
        if self.iroh_join_pending and getattr(context.iroh, "join_outcome", None) == "joined":
            return done()
