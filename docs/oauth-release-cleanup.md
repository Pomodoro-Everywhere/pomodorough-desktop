...
The target is suite **0.7.0** (`v0.7.0`). The version changes are currently uncommitted; these examples do not assert that this release exists. Desktop `800583a9bc6292ededd9f8453a4972b706c8942b` provides the previously reviewed sign-off implementation, not production acceptance evidence for the 0.7.0 candidate. Source tests, deterministic artifact self-tests, and this checklist cannot close P2.2.
...
| --- | --- | --- |
| `wheel` | `pomodorough_linux-0.7.0-py3-none-any.whl` | Linux desktop; `system=Linux`, `machine` matches the recorded host architecture |
| `flatpak` | `Pomodorough-0.7.0-x86_64.flatpak` | Linux x86_64, installed bundle with Secret Service; `system=Linux`, `machine=x86_64` |
| `windows` | `Pomodorough-0.7.0-windows-x86_64.exe` | Windows x86_64 with DPAPI; `system=Windows`, `machine=AMD64` |
